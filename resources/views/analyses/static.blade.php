...
@section('content')
        ...
        <div class="col-md-12">
            ...
            <select id="analysis_id" class="form-select mb-3">
                ...
                @foreach($analyses as $analysis)
                    <option value="{{ $analysis->id }}">{{ $analysis->parsed_data['analysis_id'] }}</option>
                @endforeach
            </select>
        </div>
    </div>
    ...
    <script>
        ...
        function generateFields(data) {
            const initialFields = ['analysis_id', 'score', 'tags', 'families'];
            const collapsibleFields = {};

            for (const key in data) {
                if (data.hasOwnProperty(key)) {
                    if (initialFields.includes(key)) {
                        if (data[key] !== null && data[key] !== "" && Object.keys(data[key]).length !== 0) {
        ...
        function generateTabs(data, analysis_id) {
            ...
            let isFirst = true;
            for (const key in data) {
                if (data.hasOwnProperty(key) && key !== 'analysis_id' && key !== 'score' && key !== 'tags' && key !== 'families') {
        ...
        function handlePopulateCards() {
            ...
            if (selectedData) {
                const analysis_id = selectedData.data.analysis_id;
                generateTabs(selectedData.data, analysis_id);

                if (selectedData.data.screenshot) {
                    setTimeout(() => {
                        handleScreenshots(selectedData.data.screenshot, analysis_id);
                    }, 1);
                }
            } else {
                dynamicFields.innerHTML = '';
        ...
        function handleScreenshots(data, analysis_id) {
            ...
            if (carouselList) {
                ...
                data.forEach((screenshot) => {
                    const imageUrl = `/analysis/${analysis_id}/screenshot/${screenshot.name}`;
